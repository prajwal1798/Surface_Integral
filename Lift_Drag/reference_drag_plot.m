...
% title(sprintf('$C_d$ Progression for Test Case %d', testCaseIdx), 'Interpreter', 'latex', 'FontSize', 16);
% legend({'Predicted Drag', 'Reference Drag'}, 'Interpreter', 'latex', 'FontSize', 12, 'Location', 'best');
% 
% % Grid and formatting
% grid on;
% set(gca, 'FontSize', 12);
...
for i = 1:length(trainingCases)
    ...
    Cd_interp_D = data_D(3, testCaseIdx);
    Cd_pred_D = data_D(4, testCaseIdx);
    
    Cd_pred_C = data_C(4, testCaseIdx);
...
plot(log10(trainingCases), predicted_drag_N200, '-^', 'Color', 'b', 'LineWidth', 2, 'MarkerSize', 8, 'MarkerEdgeColor', 'b', 'DisplayName', 'Predicted Drag N200');

% Plot for N100 (from C drive)
plot(log10(trainingCases), predicted_drag_N100, '-s', 'Color', 'k', 'LineWidth', 2, 'MarkerSize', 8, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'g', 'DisplayName', 'Predicted Drag N100');

% Plot reference drag value
yline(reference_drag, '--r', 'LineWidth', 2, 'DisplayName', 'Reference Drag');

% Labels and title with LaTeX interpreter
xlabel('$ log_{10} \,(\texttt{N}_\texttt{tr})$', 'Interpreter', 'latex', 'FontSize', 14);
ylabel('$ log_{10} \, (C_d)$', 'Interpreter', 'latex', 'FontSize', 14);
title(sprintf('Drag Progression for Test Case %d', testCaseIdx), 'Interpreter', 'latex', 'FontSize', 16);
